...
class Tree {
  private:
    ...
    void insert(Node<T> *node, T value) {
		...
		T* find(Node<T> *node, T value) const {
			T *result = NULL;
		  if (node == NULL) {
				return NULL;
			}
    ...
    void preorder_traversal(Node<T> *node) const {
		  if(node != NULL) {
		    cout << node->value << " ";
		...
		void inorder_traversal(Node<T> *node) const {
		  if(node != NULL) {
		...
		void postorder_traversal(Node<T> *node) const {
		  if(node != NULL) {
