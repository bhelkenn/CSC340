...
void Budget::traverseEnvelopeBox() const {
	currentEnvelopeBox.traverse();
}
...
void Budget::deposit_to_envelope(int account_id, double amount) {
	currentEnvelopeBox.deposit(account_id, amount);
}
bool Budget::withdraw_from_envelope(int account_id, double amount) {
	return currentEnvelopeBox.withdraw(account_id, amount);
}
...
ostream& operator << (ostream& stream, const Budget &budget) {
  ...
  stream << endl;
	...
	stream << budget.currentEnvelopeBox.size() << " Envelopes:" << endl;
	budget.currentEnvelopeBox.traverse();
	/*
...
ifstream& operator >> (ifstream& file, Budget &budget) {
  ...
  budget.account_count = atoi(tmp.c_str());
	...
	EnvelopeBox eb(budget.envelopes, 
								budget.envelope_count, 
								budget.current_month, 
								budget.current_year);
	budget.currentEnvelopeBox = eb;
